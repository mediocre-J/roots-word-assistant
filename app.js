(function() {
    'use strict';
    // 通过 Vercel Serverless Function 代理（隐藏 API Key）
    const API_PROXY_URL = '/api/proxy';
    const FALLBACK_DIRECT = false;
    ...
    const recognizeBtn = $('recognizeBtn'), clearBtn = $('clearBtn');
    ...
    const wordEditArea = $('wordEditArea'), newWordInput = $('newWordInput'), wordCountSpan = $('wordCountSpan'), clearAllWordsBtn = $('clearAllWordsBtn');
    ...
    function renderWords(groups) {
        ...
        newWordInput.onkeydown = (e) => {
            if (e.key === 'Enter') {
                const w = newWordInput.value.trim().replace(/[^a-zA-Z-]/g, '');
                if (w && w.length > 1) {
                    if (groups.length && groups[groups.length - 1].words) {
                        groups[groups.length - 1].words.push(w);
                    } else {
                        groups.push({line: '', words: [w], phrases: []});
                    }
                    renderWords(groups);
                }
                newWordInput.value = '';
    ...
    async function fetchAI(body) {
        ...
        const timeout = setTimeout(() => controller.abort(), 20000); // 20 秒超时
    ...
    async function analyzeByAI(word, retryCount) {
        if (retryCount === undefined) retryCount = 0;
        const cached = getCached(word);
        if (cached) return cached;
        const prompt = '分析英文单词 "' + word + '"，返回 JSON（不要 markdown 包裹）：\n' +
            '{\n' +
            '  "phonetic": "音标",\n' +
            '  "meaning": "中文释义（含词性，多义用分号）",\n' +
            '  "meanings": [{"meaning": "含义1（含词性）", "example": "例句+中文翻译"}],\n' +
            '  "root": "词根", "root_meaning": "词根含义",\n' +
            '  "prefix": "前缀或空", "prefix_meaning": "前缀含义或空",\n' +
            '  "suffix": "后缀或空", "suffix_meaning": "后缀含义或空",\n' +
            '  "etymology": "词源|记忆场景|含此单词的例句",\n' +
            '  "collocations": [{"text": "搭配（中文释义）"}],\n' +
            '  "synonyms": [{"word": "同义词", "meaning": "中文释义"}],\n' +
            '  "antonyms": [{"word": "反义词", "meaning": "中文释义"}],\n' +
            '  "frequency": "高频词/中频词/低频词",\n' +
            '  "related_words": [{"word": "同根词", "meaning": "含义", "part_of_speech": "词性", "frequency": "词频"}]\n' +
            '}';
        try {
            const data = await fetchAI({
                model: 'deepseek-chat',
            ...
            // 超时或失败时自动重试一次
            if (retryCount < 1) {
                console.warn('首次分析失败，自动重试:', word, e.message);
                return analyzeByAI(word, retryCount + 1);
            }
            throw e;
        }
    }
    ...
    async function handleWordAnalysis(word) {
        ...
        var slowTimer = setTimeout(function() {
            loadingBar.innerHTML = '<span class="loading-spinner" style="width:14px;height:14px;border-width:2px;"></span> ⏳ "' + word + '" 请求较慢，请耐心等待...<br><small style="opacity:0.8;">首次分析需要调用 AI，通常需要 5-15 秒</small>';
        }, 8000);
        ...
        try {
            ...
            const html = '' +
                '<div class="analysis-card-header">' +
                    '<h3>' + word + (r.part_of_speech ? ' <span style="font-size:0.7rem;font-weight:400;background:rgba(255,255,255,0.2);padding:0.15rem 0.6rem;border-radius:50px;vertical-align:middle;">' + r.part_of_speech + '</span>' : '') + phoneticHtml + speakBtnHtml + '</h3>' +
                    '<div>' +
                ...
                '<div class="analysis-card-body">' +
                    ...
                    '</div>' +
                    (r.inflections ? (function(inf) {
                        const items = [];
                        if (inf.plural) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.plural.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">复数</span><span class="value">' + inf.plural + '</span></span>');
                        if (inf.past_tense) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.past_tense.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">过去式</span><span class="value">' + inf.past_tense + '</span></span>');
                        if (inf.past_participle) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.past_participle.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">过去分词</span><span class="value">' + inf.past_participle + '</span></span>');
                        if (inf.present_participle) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.present_participle.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">现在分词</span><span class="value">' + inf.present_participle + '</span></span>');
                        if (inf.comparative) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.comparative.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">比较级</span><span class="value">' + inf.comparative + '</span></span>');
                        if (inf.superlative) items.push('<span class="inflection-item" onclick="window.speakWord(\'' + inf.superlative.replace(/'/g, "\\'") + '\')" style="cursor:pointer;"><span class="label">最高级</span><span class="value">' + inf.superlative + '</span></span>');
            ...
            // 记录查词历史
            addHistory(word);
            var pendingAnalysis = {word: word, html: html};


            // 分析完成后在页面底部显示浮动提示条（垂直堆叠，互不遮挡）
            ...
            if (!notifyContainer) {
                ...
                notifyContainer.style.cssText = 'position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);z-index:9999;display:flex;flex-direction:column-reverse;gap:0.5rem;max-width:90%;pointer-events:none;';
                document.body.appendChild(notifyContainer);
            }
            var notifyBar = document.createElement('div');
            notifyBar.id = notifyId;
            notifyBar.style.cssText = 'pointer-events:auto;text-align:center;padding:0.75rem 1.5rem;background:var(--primary);color:white;border-radius:50px;cursor:pointer;animation:fadeInUp 0.3s ease;box-shadow:0 8px 32px rgba(0,0,0,0.2);font-size:0.95rem;font-weight:500;display:flex;align-items:center;justify-content:center;gap:0.5rem;white-space:nowrap;';
            notifyBar.innerHTML = '✅ "' + word + '" 分析完成 <span style="font-size:0.75rem;opacity:0.9;">点击查看</span> <span style="margin-left:0.5rem;font-size:0.8rem;opacity:0.7;cursor:pointer;background:rgba(255,255,255,0.15);border-radius:50%;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;" onclick="event.stopPropagation();this.parentElement.remove()">✕</span>';
            notifyBar.onclick = function() {
                analysisResult.style.display = 'block';
                analysisResult.innerHTML = pendingAnalysis.html;
                notifyBar.remove();
                setTimeout(function() {
    ...
    function renderHistory() {
        ...
        list.innerHTML = history.map(function(word) {
            ...
            var meaning = cached ? (cached.meaning || '') : '';
            var meaningHtml = meaning ? ' <span style="font-size:0.7rem;color:var(--text-muted);">' + meaning + '</span>' : '';
            return '<span class="history-tag" onclick="window.handleWordAnalysis(\'' + word + '\')" title="点击重新查看">' + word + meaningHtml + '</span>';
        }).join('');
        // 默认折叠
    ...
    if (exportReviewBtn) {
        exportReviewBtn.onclick = exportReviewData;
    }
    if (importReviewBtn) {
    ...
    exportCsvBtn.onclick = () => {
        ...
        let csv = '\uFEFF单词,音标,释义,词根,词根含义,前缀,前缀含义,后缀,后缀含义,词源,例句,搭配\n';
        analyzed.forEach(w => {
            ...
            const row = [
                ...
                cached.root_meaning || '',
                cached.prefix || '',
                cached.prefix_meaning || '',
                cached.suffix || '',
                cached.suffix_meaning || '',
                cached.etymology || '',
                cached.example_sentence || '',
                (cached.collocations || []).join('; ')
    ...
    if (reUploadBtn) {
        reUploadBtn.onclick = function(e) {
            e.stopPropagation();
            // 清空 input value，确保即使选择同一文件也能触发 change 事件
            imageInput.value = '';
            // 清除旧状态，避免新旧数据混淆
            wordGroups = [];
            originalOcrText = '';
    ...
    switchInputBtn.onclick = () => {
        ...
        clearBtn.style.display = isTextMode ? (wordGroups.length > 0 ? 'inline-flex' : 'none') : (currentImageFile ? 'inline-flex' : 'none');
